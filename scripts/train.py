...
def main(args):
    ...
    if batch_size <= vertices.shape[0]:
        ...
        loader = DataLoader(
            dset, batch_size=batch_size, shuffle=True, pin_memory=True, drop_last=True
        )
    else:
        raise RuntimeError(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no_normalization", help="Normalize data before training", action="store_true"
    )
    parser.add_argument(
        "--disable_cudnn", help="Disable CUDNN during training", action="store_true"
    )
    parser.add_argument(
        "--benchmark_cudnn",
        help="Run CUDNN performance benchmarks before training",
        action="store_true",
    )
    parser.add_argument(
        "--save_epochs",
        type=int,
        default=20,
        help="Frequency of saving the network state, in epochs",
    )
    parser.add_argument("--load_path", type=str, default=None, help="PNODE state_dict file path")
    parser.add_argument(
        "--out_dims", type=int, nargs="+", default=(48, 48, 48), help="Shape of the output tensor"
    )
    parser.add_argument(
        "--encoder_layers",
        type=int,
        nargs="+",
        default=None,
        help="A sequence of encoder layer widths",
    )
    parser.add_argument(
        "--node_layers",
        type=int,
        nargs="+",
        default=None,
        help="A sequence of layer widths in the NODE kernel",
    )
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="A sequence of decoder dense layer widths",
    )
